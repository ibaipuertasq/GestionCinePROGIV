...
// Funcion privada para mostrar el encabezado
static void mostrar_encabezado(const char* titulo) {
    ...
    // Ajustar si la longitud del titulo es impar
...
// Funcion para mostrar la cartelera
static void cliente_ver_cartelera() {
    ...
    if (!pelicula_listar(&peliculas, &num_peliculas)) {
        menu_mostrar_error("Error al cargar las peliculas");
        menu_pausar();
        return;
    }
    
    if (num_peliculas == 0) {
        printf("No hay peliculas en cartelera actualmente.\n");
        menu_pausar();
        return;
    }
    ...
    // Mostrar cada pelicula con sus sesiones
    for (int i = 0; i < num_peliculas; i++) {
        printf("\n%d. %s\n", i + 1, peliculas[i].titulo);
        printf("   Genero: %s | Duracion: %d minutos\n", peliculas[i].genero, peliculas[i].duracion);
        
        // Obtener sesiones para esta pelicula
        ...
        if (sesion_buscar_por_pelicula(peliculas[i].id, &sesiones, &num_sesiones)) {
            if (num_sesiones > 0) {
                ...
                for (int j = 0; j < num_sesiones; j++) {
                    ...
                    printf("     - Sesion ID: %d | Fecha: %s | Hora: %s-%s | Sala: %d | Asientos libres: %d\n",
                           sesiones[j].id, fecha, hora_inicio, hora_fin, sala.id, asientos_libres);
                }
            } else {
                printf("   No hay sesiones disponibles para esta pelicula.\n");
            }
...
static void cliente_mostrar_asientos(int sesion_id) {
    ...
    if (!sesion_obtener_por_id(sesion_id, &sesion)) {
        menu_mostrar_error("Sesion no encontrada");
        return;
    }
    
    Sala sala;
    if (!sala_obtener_por_id(sesion.sala_id, &sala)) {
        menu_mostrar_error("Informacion de sala no disponible");
        return;
    }
    ...
    // Determinar el numero de filas y columnas para una visualizacion agradable
    ...
    for (int fila = 0; fila < filas; fila++) {
        printf("%3d   ", fila + 1);  // Numero de fila
        
        for (int col = 0; col < columnas; col++) {
            int asiento_idx = fila * columnas + col;
            
            if (asiento_idx < num_asientos) {
                // Verificar si el asiento esta disponible para esta sesion
...
// Funcion para comprar entradas (version corregida)
static void cliente_comprar_entradas() {
    ...
    if (!pelicula_listar(&peliculas, &num_peliculas) || num_peliculas == 0) {
        menu_mostrar_error("No hay peliculas disponibles");
        menu_pausar();
        return;
    }
    ...
    int opcion_pelicula = menu_leer_entero("Seleccione una pelicula", 1, num_peliculas);
    int pelicula_id = peliculas[opcion_pelicula - 1].id;
    
    // Liberar la memoria de las peliculas
    pelicula_liberar_lista(peliculas, num_peliculas);
    
    // Obtener sesiones para la pelicula seleccionada
    Sesion* sesiones = NULL;
    int num_sesiones = 0;
    
    if (!sesion_buscar_por_pelicula(pelicula_id, &sesiones, &num_sesiones) || num_sesiones == 0) {
        menu_mostrar_error("No hay sesiones disponibles para esta pelicula");
        menu_pausar();
        return;
    }
    ...
    for (int i = 0; i < num_sesiones; i++) {
        ...
        // Formatear fecha y hora para mejor visualizacion
    ...
    int opcion_sesion = menu_leer_entero("Seleccione una sesion", 1, num_sesiones);
    ...
    int num_entradas = menu_leer_entero("¿Cuantas entradas desea comprar?", 1, 10);
    ...
    if (menu_confirmar("¿Confirmar compra?")) {
        // Iniciar la transaccion para toda la venta
        ...
        if (!db_commit_transaction()) {
            log_error("Error al confirmar transaccion");
            db_rollback_transaction();
        ...
        menu_mostrar_exito("¡Compra realizada con exito!");
    } else {
...
// Mostrar informacion detallada de una venta
static void cliente_mostrar_detalle_venta(int venta_id) {
    ...
    printf("%-5s %-20s %-20s %-10s %-10s\n", 
           "ID", "Pelicula", "Fecha y Hora", "Sala", "Asiento");
    printf("-------------------------------------------------------------------------\n");
...
// Funcion para ver compras realizadas
static void cliente_ver_compras() {
    ...
    if (num_ventas == 0) {
        printf("No has realizado ninguna compra aun.\n");
        menu_pausar();
        return;
    }
...
// Funcion privada para mostrar el pie de pagina
...
void menu_limpiar_pantalla() {
    if (!g_config->clear_screen) {
        // Si esta desactivado en la configuracion, solo agregar saltos de linea
        printf("\n\n\n\n\n");
        return;
    }
...
void menu_leer_texto(char* buffer, int tamano, const char* prompt) {
    ...
    // Si el buffer esta vacio, volver a pedir
    if (strlen(buffer) == 0) {
...
int menu_leer_entero(const char* prompt, int min, int max) {
    ...
    while (1) {
        ...
        menu_mostrar_error("Valor invalido. Intente de nuevo.");
    }
}

// Leer un numero decimal
...
// Funciones para los diferentes menus

// Menu principal
static void mostrar_menu_principal() {
    menu_limpiar_pantalla();
    mostrar_encabezado("SISTEMA DE GESTION DE CINE");
    
    printf("1. Iniciar sesion\n");
    printf("2. Salir\n");
    
    int opcion = menu_leer_entero("Seleccione una opcion", 1, 2);
...
static void mostrar_menu_autenticacion() {
    ...
    menu_leer_texto(correo, sizeof(correo), "Correo electronico");
    menu_leer_texto(contrasena, sizeof(contrasena), "Contrasenya");
    
    if (auth_login(correo, contrasena)) {
        menu_mostrar_exito("Inicio de sesion exitoso");
        ...
        menu_mostrar_error("Correo o contrasenya incorrectos");
        menu_pausar();
...
static void mostrar_menu_admin() {
    while (auth_sesion_activa() && auth_es_administrador()) {
        menu_limpiar_pantalla();
        mostrar_encabezado("MENU DE ADMINISTRADOR");
        
        printf("1. Gestionar Peliculas\n");
        ...
        int opcion = menu_leer_entero("Seleccione una opcion", 1, 6);
        ...
        switch (opcion) {
            ...
            case 3:
                ...
                break;
            case 4:
                // Menu de usuarios (a implementar)
                break;
            case 5:
    ...
    // Si llegamos aqui, la sesion ha expirado
    if (!auth_sesion_activa()) {
        menu_mostrar_error("La sesion ha expirado");
        menu_pausar();
        menu_mostrar(MENU_PRINCIPAL);
    }
}
...
static void mostrar_menu_cliente() {
    while (auth_sesion_activa()) {
        menu_limpiar_pantalla();
        mostrar_encabezado("MENU DE CLIENTE");
        ...
        int opcion = menu_leer_entero("Seleccione una opcion", 1, 4);
        ...
        switch (opcion) {
            ...
            case 4:
                auth_logout();
                menu_mostrar_exito("Sesion cerrada correctamente");
                menu_pausar();
                menu_mostrar(MENU_PRINCIPAL);
                return;
        }
...
static void mostrar_menu_peliculas() {
    while (auth_sesion_activa() && auth_es_administrador()) {
        menu_limpiar_pantalla();
        mostrar_encabezado("GESTION DE PELICULAS");
        ...
        switch (opcion) {
            case 1: {
                ...
                menu_limpiar_pantalla();
                mostrar_encabezado("LISTA DE PELICULAS");
                ...
                if (pelicula_listar(&peliculas, &num_peliculas)) {
                    ...
                    menu_mostrar_error("Error al listar las peliculas");
                }
            ...
            case 2: {
                // Anyadir pelicula
                menu_limpiar_pantalla();
                mostrar_encabezado("ANYADIR PELICULA");
                
                Pelicula nueva_pelicula = {0};
                
                menu_leer_texto(nueva_pelicula.titulo, sizeof(nueva_pelicula.titulo), "Titulo");
                nueva_pelicula.duracion = menu_leer_entero("Duracion (minutos)", 1, 500);
                menu_leer_texto(nueva_pelicula.genero, sizeof(nueva_pelicula.genero), "Genero");
                
                if (pelicula_crear(&nueva_pelicula)) {
                    menu_mostrar_exito("Pelicula creada correctamente");
                } else {
                    menu_mostrar_error("Error al crear la pelicula");
                }
            ...
            case 3: {
                // Editar película
                menu_limpiar_pantalla();
                mostrar_encabezado("EDITAR PELICULA");
                
                int id = menu_leer_entero("ID de la pelicula a editar", 1, 9999);
                Pelicula pelicula = {0};
                ...
                if (pelicula_obtener_por_id(id, &pelicula)) {
                    ...
                    menu_leer_texto(buffer, sizeof(buffer), "Nuevo titulo (o Enter para mantener)");
                    if (strlen(buffer) > 0) {
                    ...
                    menu_leer_texto(buffer, sizeof(buffer), "Nuevo genero (o Enter para mantener)");
                    if (strlen(buffer) > 0) {
                    ...
                    if (pelicula_actualizar(&pelicula)) {
                        ...
                        menu_mostrar_error("Error al actualizar la pelicula");
                    }
                } else {
                    menu_mostrar_error("Pelicula no encontrada");
                }
            ...
            case 4: {
                // Eliminar pelicula
                menu_limpiar_pantalla();
                mostrar_encabezado("ELIMINAR PELICULA");
                
                int id = menu_leer_entero("ID de la pelicula a eliminar", 1, 9999);
                Pelicula pelicula = {0};
                
                if (pelicula_obtener_por_id(id, &pelicula)) {
                    printf("Película encontrada: %s\n", pelicula.titulo);
                    
                    if (menu_confirmar("¿Esta seguro de que desea eliminar esta pelicula?")) {
                        if (pelicula_eliminar(id)) {
                            menu_mostrar_exito("Pelicula eliminada correctamente");
                        } else {
                            menu_mostrar_error("Error al eliminar la pelicula");
                        }
                    } else {
            ...
            case 5: {
                // Buscar pelicula
                menu_limpiar_pantalla();
                mostrar_encabezado("BUSCAR PELICULA");
                
                printf("1. Buscar por titulo\n");
                printf("2. Buscar por genero\n");
                
                int tipo_busqueda = menu_leer_entero("Seleccione una opcion", 1, 2);
                ...
                if (tipo_busqueda == 1) {
                    menu_leer_texto(termino, sizeof(termino), "Titulo (o parte del titulo)");
                    if (pelicula_buscar_por_titulo(termino, &peliculas, &num_peliculas)) {
                        // Mostrar resultados
                    }
                } else {
                    menu_leer_texto(termino, sizeof(termino), "Genero");
                ...
                if (num_peliculas > 0) {
                    ...
                    printf("No se encontraron peliculas con ese criterio\n");
                }
...
// Menu de salas (anyadir a menu.c)
static void mostrar_menu_salas() {
    while (auth_sesion_activa() && auth_es_administrador()) {
        menu_limpiar_pantalla();
        mostrar_encabezado("GESTION DE SALAS");
        ...
        switch (opcion) {
            case 1: {
                ...
                if (sala_listar(&salas, &num_salas)) {
                    if (num_salas > 0) {
                        printf("%-5s %-20s %-20s\n", "ID", "Numero de Asientos", "Asientos Libres");
                        printf("--------------------------------------------------\n");
            ...
            case 2: {
                // Anyadir sala
                menu_limpiar_pantalla();
                mostrar_encabezado("ANYADIR SALA");
                
                Sala nueva_sala = {0};
                nueva_sala.numero_asientos = menu_leer_entero("Numero de asientos", 1, 1000);
            ...
            case 3: {
                ...
                if (sala_obtener_por_id(id, &sala)) {
                    printf("Sala encontrada. Numero de asientos actual: %d\n", sala.numero_asientos);
                    
                    sala.numero_asientos = menu_leer_entero("Nuevo numero de asientos", 1, 1000);
            ...
            case 4: {
                ...
                if (sala_obtener_por_id(id, &sala)) {
                    printf("Sala encontrada. Numero de asientos: %d\n", sala.numero_asientos);
                    
                    if (menu_confirmar("¿Esta seguro de que desea eliminar esta sala?")) {
                        if (sala_eliminar(id)) {
...
// Menu de sesiones (añadir a menu.c)
static void mostrar_menu_sesiones() {
    while (auth_sesion_activa() && auth_es_administrador()) {
        menu_limpiar_pantalla();
        mostrar_encabezado("GESTION DE SESIONES");
        
        printf("1. Ver todas las sesiones\n");
        printf("2. Anyadir sesion\n");
        printf("3. Editar sesion\n");
        printf("4. Eliminar sesión\n");
        printf("5. Buscar sesiones por pelicula\n");
        printf("6. Buscar sesiones por sala\n");
        printf("7. Buscar sesiones por fecha\n");
        printf("8. Volver al menu anterior\n");
        
        int opcion = menu_leer_entero("Seleccione una opcion", 1, 8);
        ...
        switch (opcion) {
            ...
            case 2: {
                // Anyadir sesion
                menu_limpiar_pantalla();
                mostrar_encabezado("ANYADIR SESION");
                ...
                printf("\nIntroduzca los datos de la sesion:\n");
                
                nueva_sesion.pelicula_id = menu_leer_entero("ID de la pelicula", 1, 9999);
                ...
                // Calcular la hora de finalizacion (hora inicio + duracion pelicula + 15 min para limpieza)
                struct tm tm_inicio;
                if (!sesion_convertir_str_a_time(nueva_sesion.hora_inicio, &tm_inicio)) {
                    menu_mostrar_error("Formato de fecha/hora invalido");
                    pelicula_liberar_lista(peliculas, num_peliculas);
                    sala_liberar_lista(salas, num_salas);
                    menu_pausar();
                    break;
                ...
                time_t tiempo_fin = tiempo_inicio + (pelicula.duracion + 15) * 60; // Duracion en segundos + 15 min
                ...
                printf("Hora de finalizacion calculada: %s\n", nueva_sesion.hora_fin);
                
                if (sesion_crear(&nueva_sesion)) {
                    menu_mostrar_exito("Sesion creada correctamente");
                } else {
                    menu_mostrar_error("Error al crear la sesion (compruebe que la sala este disponible en ese horario)");
                }
            ...
            // Aqui solo muestro el caso 1 y 2 por brevedad, pero aplicarias
            // el mismo patron para los casos restantes
            ...
            case 8:
                ...
                return;
            default:
                menu_mostrar_error("Opcion no valida");
                menu_pausar();
                break;
        }
...
// Ejecutar el bucle principal del menu
void menu_ejecutar() {
    while (g_menu_activo) {
        menu_mostrar(MENU_PRINCIPAL);
    }
...
void menu_mostrar(TipoMenu tipo) {
    switch (tipo) {
        ...
        case MENU_REPORTES:
            ...
            break;
        default:
            menu_mostrar_error("Tipo de menu desconocido");
            break;
    }
}
